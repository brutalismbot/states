...
terraform {
  ...
  cloud {
    ...
    workspaces { name = "state-machines" }
  }
...
locals {
  ...
  tags = {
    App  = "Brutalismbot"
    Name = "Brutalismbot"
    Repo = "https://github.com/brutalismbot/states"
  }
}
...
data "aws_iam_policy_document" "slack_beta_app_home_opened" {
  statement {
    sid     = "DynamoDB"
  ...
  statement {
    ...
    resources = [data.terraform_remote_state.events.outputs.rules.reddit_dequeue.arn]
  }
...
module "slack_beta_app_home_opened" {
  ...
  policy = data.aws_iam_policy_document.slack_beta_app_home_opened.json
